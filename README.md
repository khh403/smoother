...
[![GoDoc](https://godoc.org/github.com/khh403/smoother?status.svg)](https://godoc.org/github.com/khh403/smoother)   [![Tests](https://github.com/khh403/smoother/workflows/Tests/badge.svg)](https://github.com/khh403/smoother/actions?workflow=Tests)

`smoother` is a package for creating monitorable, gracefully restarting, self-upgrading binaries in Go (golang). The main goal of this project is to facilitate the creation of self-upgrading binaries which play nice with standard process managers, secondly it should expose a small and simple API with reasonable defaults.
...
```sh
go get github.com/khh403/smoother
```
...
//create another main() to run the smoother process
...
* `smoother` uses the main process to check for and install upgrades and a child process to run `Program`.
...
* The `fetcher.HTTP` accepts a `URL`, it polls this URL with HEAD requests and until it detects a change. On change, we `GET` the `URL` and stream it back out to `smoother`. See also `fetcher.S3`.
* Once a binary is received, it is run with a simple echo token to confirm it is a `smoother` binary.
* Except for scheduled restarts, the active child process exiting will cause the main process to exit with the same code. So, **`smoother` is not a process manager**.

See [Config](https://godoc.org/github.com/khh403/smoother#Config)uration options [here](https://godoc.org/github.com/khh403/smoother#Config) and the runtime [State](https://godoc.org/github.com/khh403/smoother#State) available to your program [here](https://godoc.org/github.com/khh403/smoother#State).
...
* The master process's `smoother.Config` cannot be changed via an upgrade, the master process must be restarted.
...
* [Core `smoother` package](https://godoc.org/github.com/khh403/smoother)
* [Common `fetcher.Interface`](https://godoc.org/github.com/khh403/smoother/fetcher#Interface)
    * [File fetcher](https://godoc.org/github.com/khh403/smoother/fetcher#File)
    * [HTTP fetcher](https://godoc.org/github.com/khh403/smoother/fetcher#HTTP)
    * [S3 fetcher](https://godoc.org/github.com/khh403/smoother/fetcher#S3)
    * [Github fetcher](https://godoc.org/github.com/khh403/smoother/fetcher#Github)
