...
var (
	...
	//smoother is disabled and the program function
	//is run manually.
	DisabledState = State{Enabled: false}
)

// State contains the current run-time state of smoother
type State struct {
	//whether smoother is running enabled. When enabled,
	//this program will be running in a child process and
	//smoother will perform rolling upgrades.
	Enabled bool
...
//a smoother slave process

type slave struct {
	*Config
	id         string
	listeners  []*smootherListener
	masterPid  int
...
func (sp *slave) initFileDescriptors() error {
	...
	sp.listeners = make([]*smootherListener, numFDs)
...
func (sp *slave) debugf(f string, args ...interface{}) {
	if sp.Config.Debug {
		log.Printf("[smoother slave#"+sp.id+"] "+f, args...)
	}
}
